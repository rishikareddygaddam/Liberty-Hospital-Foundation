...
export default function PatientAssistance() {
  ...
  useEffect(() => {
    async function getData() {
      let res = await get("patient");
      if (Object.keys(res.data).length) {
        setInitial({ ...res.data, type: "patient" });
        setKey(+new Date());
      }
    }
  ...
  return (
    <div className="patient-assistance">
      <Helmet>
        <title>Liberty Hospital Foundation - Patient Assistance</title>
      </Helmet>
      <Header />
      <div className="main-container" key={key}>
        <h3 className="title">Patient Assistance</h3>
        <div className="download-container">
          <button onClick={() => download("patient")}>Export to excel</button>
        </div>
        <Formik
