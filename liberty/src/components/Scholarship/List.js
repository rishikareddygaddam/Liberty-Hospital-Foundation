...
import { list, deleteGrant } from "../../api/scholarship";
import { Link } from "react-router-dom";
import { download } from "api/scholarship";
...
function List() {
  ...
  return (
    <>
      <Helmet>
        <title>
          Liberty Hospital Foundation - Review Scholarship Applications
        </title>
      </Helmet>
      <Header />
          ...
          <tbody>
            {grants.map((e) => (
              <tr key={e.id}>
                ...
                <td>{e.id}</td>
                <td>
                  <Link to={`/scholarship/${e.id}`}>Link</Link>
                </td>
